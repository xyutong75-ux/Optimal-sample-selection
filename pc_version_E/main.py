...
def get_base_dir() -> str:
    """Get result save directory (exe dir when frozen, script dir in dev)."""
...
def parse_int(value, name, errors, min_value=None, max_value=None):
    ...
    if min_value is not None and iv < min_value:
        errors.append(f"{name} cannot be less than {min_value}.")
    if max_value is not None and iv > max_value:
        errors.append(f"{name} cannot be greater than {max_value}.")
    return iv
...
def parse_samples(raw_text, n, mode):
    ...
    if not raw_text:
        raise ValueError("In manual mode, the sample list cannot be empty.")
    separators = [",", " ", "\n", "\r", "\t", ";", "，", "；"]
    text = raw_text
    ...
    if len(items) != n:
        raise ValueError(f"Manual sample count is {len(items)}, which does not match n={n}.")
    return items
...
def parse_special_indices(raw_text, n, j):
    ...
    if len(tokens) != j:
        raise ValueError(f"Number of key sample indices is {len(tokens)}; expected j={j}.")
    indices = []
    for t in tokens:
        try:
            idx = int(t)
        except ValueError:
            raise ValueError("Key sample indices must be integers in the range 1~n.")
        if not (1 <= idx <= n):
            raise ValueError(f"Key sample index {idx} is out of range 1~{n}.")
        pos = idx - 1
        if pos in indices:
            raise ValueError(f"Duplicate key sample index found: {idx}.")
        indices.append(pos)
    return indices
...
class SampleSelectionApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Optimal Sample Selection System")
    ...
    def _build_ui(self):
        ...
        title_label = ttk.Label(
            main_frame,
            text="Optimal Sample Selection System",
        ...
        sub_label = ttk.Label(
            main_frame,
            text="Generate sample combinations by enumeration under constraints; results are auto-saved to the local 'results' folder.",
        ...
        content.add(self.page_main, text="Run")
        content.add(self.page_history, text="History")
    ...
    def _build_main_page(self, parent):
        ...
        add_labeled_entry(
            0,
            0,
            "Output group count m (recommended 45-54)",
            self.var_m,
            hint="Limits the number of groups shown/saved.",
        )
        add_labeled_entry(
            0,
            1,
            "Total sample count n (7-25)",
            self.var_n,
        )
        add_labeled_entry(
            0,
            2,
            "Samples per group k (4-7)",
            self.var_k,
        )
        add_labeled_entry(
            1,
            0,
            "Key sample count j (s <= j <= k)",
            self.var_j,
            hint="Number of pre-designated key samples among n samples.",
        )
        add_labeled_entry(
            1,
            1,
            "Minimum key samples per group s (3-7)",
            self.var_s,
            hint="Each k-sample group must contain at least s key samples.",
        )
        # 新增 t 参数输入框
        add_labeled_entry(
            1,
            2,  # 放在第一行第三列
            "Each j-tuple must be covered by at least t different k-tuples",
            self.var_t,
            hint="Default: t=1",
        )
        mode_frame = ttk.LabelFrame(parent, text="Sample Settings")
        ...
        ttk.Radiobutton(
            radio_frame,
            text="Auto-generate n samples (S1, S2, ...)",
        ...
        label_samples = ttk.Label(
            manual_frame,
            text="Manual sample list (only used in manual mode; multiple separators supported):",
        )
        ...
        hint_samples = ttk.Label(
            manual_frame,
            text="Example: A1, A2, A3, ... (comma, space, newline, etc. are supported)",
            font=("Microsoft YaHei", 8),
            foreground="#888888",
        )
        ...
        ttk.Label(
            idx_frame,
            text="Key sample indices (1~n, total j values, separated by comma/space):",
        ).pack(anchor="w")
        entry_idx = ttk.Entry(idx_frame, textvariable=self.var_special_indices)
        entry_idx.pack(fill=tk.X, pady=(2, 0))
        ttk.Label(
            idx_frame,
            text="Leave blank to use first j samples by default; otherwise provide exactly j indices in range 1~n.",
            font=("Microsoft YaHei", 8),
            foreground="#888888",
        ...
        btn_clear = ttk.Button(btn_frame, text="Clear Inputs", command=self.clear_inputs)
        btn_clear.pack(side=tk.RIGHT, padx=(0, 6))

        btn_run = ttk.Button(btn_frame, text="Run Enumeration and Save Results", command=self.run_algorithm)
        ...
        result_frame = ttk.LabelFrame(parent, text="Current Run Results")
        ...
        ttk.Label(count_frame, text="Result Count:", font=("Microsoft YaHei", 12, "bold")).pack(side=tk.LEFT)
        ...
        ttk.Label(count_frame, text="groups", font=("Microsoft YaHei", 12, "bold")).pack(side=tk.LEFT)
        ...
        self.var_result_summary = tk.StringVar(value="Not calculated yet. Click 'Run Enumeration and Save Results'.")
        ...
        self.lbl_list_title = ttk.Label(
            result_frame,
            text="All enumerated groups (k samples per group, scroll to view all):",
    ...
    def _build_history_page(self, parent):
        ...
        btn_refresh = ttk.Button(top_frame, text="Refresh History", command=self.refresh_history)
        ...
        lbl_tip = ttk.Label(
            top_frame,
            text=f"Result files are saved in: {RESULTS_DIR}",
            font=("Microsoft YaHei", 8),
            foreground="#777777",
        )
        ...
        self.tree_history.heading("params", text="Params (m, n, k, j, s)")
        self.tree_history.heading("n", text="Sample n")
        self.tree_history.heading("count", text="Group Count")
        ...
        btn_view = ttk.Button(btn_frame, text="View Selected Record", command=self.view_selected_history)
        btn_view.pack(side=tk.LEFT, padx=(4, 4))

        btn_delete = ttk.Button(btn_frame, text="Delete Selected Record", command=self.delete_selected_history)
    ...
    def clear_inputs(self):
        ...
        self.lbl_list_title.config(text="All enumerated groups (k samples per group, scroll to view all):")
        self.var_result_summary.set("Inputs cleared.")
    ...
    def run_algorithm(self):
        ...
        if m is not None and (m < 1 or m > 10000):
            errors.append("m is recommended to be between 1 and 10000 (typically 45-54).")
        if j is not None and s is not None and j < s:
            errors.append("Constraint required: j >= s.")
        if k is not None and j is not None and j > k:
            errors.append("Constraint required: j <= k.")
        if n is not None and k is not None and k > n:
            errors.append("Constraint required: k <= n.")
            # 新增 t 参数验证
        if t is not None and t < 1:
            errors.append("Constraint required: t >= 1.")
        if errors:
        ...
        self.var_result_summary.set(f"Saved to file: {filename} (Elapsed: {elapsed_ms} ms)")
        self.lbl_list_title.config(
            text=f"All {n_group} enumerated groups are listed below (scroll to view):"
        )
        self.lbl_result_footer.config(
            text=f"Displayed all {n_group} groups. Elapsed: {elapsed_ms} ms"
        )
    ...
    def show_result_popup(self, code: str, groups, elapsed_ms: int):
        ...
        win.title(f"Result Preview - {code}")
        ...
        hint = ttk.Label(
            win,
            text="All enumerated groups from this run are listed below (one per line, scrollable):",
            font=("Microsoft YaHei", 9),
            foreground="#555555",
        )
        ...
        def on_print():
            ...
            try:
                ...
                messagebox.showinfo("Export Successful", f"Results exported to text file:\n{path}\nOpen with a text editor or Word to print.")
            except Exception as e:
                messagebox.showerror("Export Failed", f"Unable to write file: {e}")
    ...
    def get_selected_history_filename(self):
        ...
        if not selection:
            messagebox.showwarning("Notice", "Please select a record in History first.")
            return None
        return selection[0]
    ...
    def view_selected_history(self):
        ...
        if not os.path.isfile(path):
            messagebox.showerror("Error", "The selected result file does not exist.")
            self.refresh_history()
            return
        try:
            with open(path, "r", encoding="utf-8") as f:
            ...
            messagebox.showerror("Error", "Failed to read the result file.")
            return

        params = data.get("params", {})
        groups = data.get("groups", [])
        special_indices = data.get("special_indices", [])
        display_indices = ", ".join(str(i + 1) for i in special_indices) or "(default: first j)"

        win = tk.Toplevel(self.root)
        win.title(f"View Result - {filename}")
        win.geometry("720x520")
        ...
        info = ttk.Label(
            ...
            text=(
                ...
                f"Parameters: m={params.get('m')}, n={params.get('n')}, "
                f"k={params.get('k')}, j={params.get('j')}, s={params.get('s')}\n"
                f"Key sample indices: {display_indices}\n"
    ...
    def delete_selected_history(self):
        ...
        if not filename:
            return
        if not messagebox.askyesno("Confirm Delete", f"Delete this record file?\n{filename}"):
            return
        path = os.path.join(RESULTS_DIR, filename)
        try:
            if os.path.isfile(path):
                os.remove(path)
            self.refresh_history()
            messagebox.showinfo("Done", "Record deleted.")
        except Exception as e:
        ...
        if os.path.isdir(RESULTS_DIR):
            for name in os.listdir(RESULTS_DIR):
                ...
                try:
                    ...
                    continue
        self.refresh_history()
        messagebox.showinfo("Done", f"Deleted {removed} history record files.")
